...
|`coproc`|在后台模式中生成子shell，并指定指定的命令|
|`cp`|将指定文件复制到另一个位置|
|`crontab`|初始化用户的crontable文件对应的编辑器|
|`cut`|删除文件行中指定的位置|
|`date`|以各种格式显示日期|
|`df`|显示所有挂载设备的当前磁盘使用情况|
|`du`|显示指定文件路径的磁盘使用情况|
|`emacs`|调用emacs文本编辑器|
|`file`|查看指定文件的文件类型|
|`find`|对文件进行递归查找|
...
|`groupadd`|创建新的系统组|
|`groupmod`|修改已有的系统组|
|`gzip`|采用lempel-ziv编码的GUN项目压缩工具|
|`head`|显示指定文件内容的开头部分|
|`help`|显示bash内建命令的帮助页面|
|`killall`|根据进程名向运行中的进程发送一个系统的信号|
|`kwrite`|调用kwrite文本编辑器|
|`less`|查看文件内容的高级用法|
|`link`|用别名创建一个指定文件的链接|
|`ln`|创建针对指定文件的符号链接或硬连接|
|`ls`|列出目录内容|
...
|`rmdir`|删除指定目录|
|`sed`|使用编辑器命令的文本流行编辑器|
|`sleep`|在指定的一段时间内暂停bash shell 的操作|
...
|`whereis`|显示指定命令的相关文件，包括二进制文本，源代码文件以及手册页|
|`which`|查找可执行文件的位置|
|`who`|显示当前系统中的登陆用户|
|`whoami`|显示当前用户的用户名|
...
### bash shell环境变量
+ 可以用set内建命令显示这些环境变量，对于不同的linux系统，开机时设置的默认shell环境变量经常会不一样；
...
|`COMP_WORDBREAKS`|在进行单词补全时用作单词分隔符的一组字节|
|`COMP_WORDS`|含有当前命令行上所有单词的数组|
|`COMPREPLY`|含有由shell函数生成的可能补全码的数组|
|`COPROC`|含有用于匿名进程I/O的文件描述符的数组|
...
|`HOSTNAME`|当前主机的名称|
|`HOSTTYPE`|当前运行bash shell 的机器|
|`IGNOREEOF`|shell在退出前必须收到连续的EOF字符的数量，如果这个值不存在，默认是1|
|`INPUTRC`|readline起始化文件名默认为.inputec|
|`LANG`|shell的语言环境分类|
|`LC_ALL`|定义一个语言环境分类，他会覆盖LANG变量|
|`LC_COLLATE`|设置对字符串值排序时用的对照表的顺序|
|`LC_CTYPE`|决定在文件名扩展和模式匹配时，如何解释其中的字符|
|`LC_MESSAGES`|决定解释前置美元符的双引号字符串的语言环境设置|
|`LC_NUMERIC`|决定格式化数字时的所使用的语言环境设置|
|`LINENO`|脚本中当前执行代码的代号|
|`LINES`|定义了终端上可见的行数|
|`MACHTYPE`|用cpu-公司-系统格式定义的系统类型|
|`MAILCHECK`|shell多久查看一次新邮件以秒为单位默认为60s|
|`MAPFILE`|含有mapfile命令所读入文本的数组，当没有给出变量名的时候，使用该环境变量|
|`OLDPWD`|shell之前的工作目录|
|`OPTERR`|设置为1时，bash shell 显示getopts命令产生的错误|
...
|`PS2`|次命令行提示符字符串|
|`PS3`|select命令的提示符|
|`PS4`|如果使用了bash的-x选项，在命令行显示之前的提示符|
|`PWD`|当前工作目录|
|`RANOM`|返回一个0~32767的随机数，对其赋值可作为随机数生成器的种子|
|`READLINE_LINE`|保存了readline行缓冲区的内容|
|`READLINE_POINT`|当前readline行缓冲区的插入点位置|
|`REPLY`|read命令的默认变量|
|`SECONDS`|自shell启动到现在的秒数，对其赋值将会重置计时器|
|`SHELL`|shell的全路径名|
|`SHELLOPTS`|已启动bash shell选项列表，由冒号分隔|
|`SHLVL`|表明shell层级，每次启动一个新的bash shell计时器加1|
|`TIMEFORMAT`|指定了shell显示的时间值的格式|
|`TMOUT`|select和read命令在没输入的情况下等待多久以秒为单位，默认值是0.表示无限长|
